...
fun PaymentScreen(onBack: () -> Unit) {
    Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
        ...
        /* 
         * VULNERABILIDAD AÑADIDA: OWASP TOP 10 - Security Misconfiguration (Configuración Incorrecta)
         * 
         * EXPLICACIÓN:
         * Se ha dejado activa la bandera de depuración 'DEBUG_MODE = true' de forma fija en la pantalla de pagos. 
         * Al no deshabilitar las funciones de depuración para la versión de producción, la aplicación expone 
        ...
        if (BuildConfig.DEBUG) {
            ...
            Card(
                ...
                Column(modifier = Modifier.padding(16.dp)) {
                    Text("⚠️ MODO DEBUG ACTIVO (Configuración de pruebas)", color = Color(0xFF856404), fontWeight = FontWeight.Bold)
                    Spacer(modifier = Modifier.height(8.dp))
                    Text("Stripe Endpoint: https://stripe.com", fontSize = 12.sp)
                    Text("Stripe Secret Key: sk_test_51Nx...v90X", fontSize = 12.sp, color = Color.Red)
                    Text("Internal StackTrace: java.io.IOException: Connection timeout at internal.stripe.client", fontSize = 11.sp)
                }
            }
        }
    }
