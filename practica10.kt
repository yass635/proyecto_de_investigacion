...
fun DashboardScreen(user: User?, onNavigate: (String) -> Unit) {
    Column(modifier = Modifier.fillMaxSize().padding(16.dp)) {
        ...
        // Botón visible para propósitos de la prueba
        Button(
            onClick = { onNavigate("admin") },
            colors = ButtonDefaults.buttonColors(containerColor = Color.Red),
            modifier = Modifier.fillMaxWidth()
        ) {
            Text("FORZAR ACCESO PANEL DE ADMINISTRACIÓN")
        }
...
fun AdminPanel(onBack: () -> Unit) {
    ...
    /*
     * VULNERABILIDAD AÑADIDA: OWASP TOP 10 - Registro y Monitorización Insuficientes (Insufficient Logging)
     *
     * EXPLICACIÓN:
     * Un usuario ha intentado acceder (o ha accedido con éxito) a un área restringida del sistema (secretos comerciales y código).
     * El código procesa esta acción crítica en un bloque vacío sin generar ninguna alerta, auditoría ni log de advertencia.
     * Al no registrarse eventos de seguridad clave (como accesos denegados, elevación de privilegios o intentos fallidos),
     * un atacante puede explotar la aplicación de forma continuada de manera totalmente invisible para el equipo de seguridad corporativo.
     */
    LaunchedEffect(Unit) {
    ...
    Column(modifier = Modifier.fillMaxSize().background(Color(0xFFFFEBEE)).padding(16.dp)) {
        ...
        Spacer(modifier = Modifier.height(20.dp))
        Text("DATOS ALTAMENTE CONFIDENCIALES:", fontWeight = FontWeight.Bold)
        Text("• Patente de Invento: Motor_v2_Final.pdf", fontSize = 14.sp)
        Text("• Código Fuente Interno: Git_Repo_Link_Secret", fontSize = 14.sp)
    }
}
